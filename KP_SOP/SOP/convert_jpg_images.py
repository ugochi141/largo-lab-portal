...
# Updated image paths with JPG extensions (converted from PNG)
base_path = "/Users/ugochi141/Desktop/KP SOP/HEME/URINES/IMG/Needs to be SOP"
image_paths = {
    'novus_uf': os.path.join(base_path, 'NovusandUF.jpg'),
    'control_panel': os.path.join(base_path, 'Screenshot 2025-07-22 at 6.31.19 PM.jpg'),
    'color_guide': os.path.join(base_path, 'Screenshot 2025-07-30 at 8.28.20 PM.jpg'),
    'sg_clean': os.path.join(base_path, 'Screenshot 2025-07-22 at 5.18.09 PM.jpg'),
    'waste_opening': os.path.join(base_path, 'Screenshot 2025-07-22 at 6.30.15 PM.jpg'),
    'cassette_handling': os.path.join(base_path, 'novus-cassette-16x9_1800000000882813.jpg'),
    'manual_full': os.path.join(base_path, 'urine micrscope analysis full.jpg'),
    'manual_analysis': os.path.join(base_path, 'urine micrscope analysis.jpg'),
    'manual_analysis2': os.path.join(base_path, 'urine micrscope analysis2.jpg'),
    'microscopic_analysis': os.path.join(base_path, 'DBDED27B-5D73-4606-8883-4DBBD4618EA7.jpeg'),
    'microscope_findings': os.path.join(base_path, 'urine microscope findings.jpg'),
    'crystals_analysis': os.path.join(base_path, 'urine micrscope analysis crystals.jpg'),
    'urine_crystals': os.path.join(base_path, 'urine crstals.jpg'),
    'urine_casts': os.path.join(base_path, 'Urine cast .jpg'),
    'extra_tube': os.path.join(base_path, 'Extra Tube.jpeg')
}
...
for key, path in image_paths.items():
    print(f"Processing {key}: {os.path.basename(path)}")
...
print(f"\nTotal images successfully processed: {len([k for k, v in base64_images.items() if v])}")
...
print(f"\n🎯 COMPLETE HTML FILE UPDATED!")
print(f"📁 Path: {output_path}")
print(f"🖼️ Total images embedded: {len([k for k, v in base64_images.items() if v])}")
print(f"❌ Failed images: {len([k for k, v in base64_images.items() if not v])}")

if len([k for k, v in base64_images.items() if not v]) > 0:
    print("\n⚠️ Images that couldn't be converted:")
    for k, v in base64_images.items():
        if not v:
            print(f"   - {k}")

print("\n✅ COMPLETED TASKS:")
print("✓ All available JPG images converted to base64")
print("✓ Images properly placed in requested sections")
print("✓ HTML file updated with complete functionality")
